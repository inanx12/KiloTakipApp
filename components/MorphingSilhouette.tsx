...
/**
 * VKİ'ye göre genişleyen / boya göre uzayan parametrik silüet.
 *
 * NOT: Transform'lar STATİK string olarak veriliyor. Daha önce reanimated
 * `useAnimatedProps` ile SVG'ye sürekli değişen string `transform` yazılıyordu;
 * bu kombinasyon react-native-svg ile native'de çökmeye yol açıyordu
 * (gerçek profil girilip ölçek değişince). Statik transform native-güvenli ve
 * görünüm birebir aynı; morph, profil değişince anında oturur.
 */
export function MorphingSilhouette({ bmi, heightCm }: MorphingSilhouetteProps) {
  ...
  const scaleX = Math.max(0.6, Math.min(1.8, activeBmi / BASELINE_BMI));
  const scaleY = Math.max(0.8, Math.min(1.2, activeHeight / BASELINE_HEIGHT));
  const headScaleX = 1 + (scaleX - 1) * 0.2;
  ...
  return (
    <View className="items-center justify-center bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-3xl p-6 relative overflow-hidden">
      ...
      <Svg width="200" height="280" viewBox="0 0 200 300" className="z-10">
        <G transform={`translate(0, 150) scale(1, ${scaleY}) translate(0, -150)`}>
          <Ellipse
            cx="100"
            cy="42"
            rx={14 * headScaleX}
          ...
          />
          <Path
            transform={`translate(100, 0) scale(${scaleX}, 1) translate(-100, 0)`}
      ...
      <View className="flex-row justify-between items-center w-full mt-4 border-t border-light-border dark:border-dark-border pt-3 z-10">
        <View>
          ...
          <Text className="text-sm font-black text-light-text dark:text-white mt-0.5">
            {scaleX.toFixed(2)}x <Text className="text-xs font-normal text-light-subtext dark:text-dark-subtext">({bmi > 0 ? "VKİ" : "Hedef"})</Text>
          </Text>
        </View>
        ...
        <View className="items-end">
          ...
          <Text className="text-sm font-black text-light-text dark:text-white mt-0.5">
            {scaleY.toFixed(2)}x <Text className="text-xs font-normal text-light-subtext dark:text-dark-subtext">({activeHeight} cm)</Text>
          </Text>
        </View>
      </View>
    </View>
